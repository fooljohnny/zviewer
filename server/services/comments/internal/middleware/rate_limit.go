...
// CommentRateLimitMiddleware creates a rate limiting middleware specifically for comment creation
func CommentRateLimitMiddleware(rateLimit rate.Limit, burst int) gin.HandlerFunc {
	manager := NewRateLimitManager(rateLimit, burst, 5*time.Minute)
