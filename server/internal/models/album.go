...
type Album struct {
	...
	UserName           string                 `json:"userName" db:"-"` // Computed field
...
func NewAlbum(title, description, userID, userName string, imageIDs []string, tags []string, isPublic bool) *Album {
	now := time.Now()
	return &Album{
		ID:          uuid.New().String(),
		Title:       title,
...
func (a *Album) UpdateCover(imageID, imagePath, thumbnailPath string) {
	...
	// Only set thumbnail path if it's not empty
	if thumbnailPath != "" {
		a.CoverThumbnailPath = &thumbnailPath
	} else {
		a.CoverThumbnailPath = nil
	}
